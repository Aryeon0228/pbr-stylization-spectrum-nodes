...
이 저장소는 석사학위논문 「⟨PBR 텍스처에서 사실적 표현과 양식화 표현의 인지 전환점 연구
⟩」(김소연, 홍익대학교 대학원 게임콘텐츠전공, ⟨연도⟩)의
실험 자극물을 생성하는 데 사용한 Substance 3D Designer 절차적 노드 그래프를 공개합니다.
연구 결과의 재현 가능성을 확보하고, 후속 연구가 동일한 자극물 생성 파이프라인을
...
본 연구는 PBR(물리 기반 렌더링) 텍스처가 리얼리스틱에서 스타일라이즈드로
...
이 저장소의 노드 그래프는 그 자극물을 만들어 낸 생성 파일 입니다.
...
- Adobe Substance 3D Designer ⟨16.0.1⟩ 이상
...
> 김소연. (⟨2026⟩). *PBR 텍스처 양식화 스펙트럼 — Substance Designer 노드 그래프* [Data set].
...
© ⟨2026⟩ 김소연 (Kim Soyoun). Licensed under CC BY 4.0.

## 연락처

- 김소연 — ⟨cloudysnowyday@gmail.com⟩
...
"⟨A Study on Perceptual Transition Points between Realistic and Stylized Representations in PBR Textures
⟩" (Soyoun Kim, Dept. of Game Content, Graduate School,
...
Each `.sbs` file requires Adobe Substance 3D Designer ⟨16.0.1⟩ or later.
